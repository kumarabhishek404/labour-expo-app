...
const EmployerBookings = () => {
  ...
  const {
    ...
    queryKey: ["employerServices", userDetails?._id],
    queryFn: async ({ pageParam = 1 }) => {
      const token = await getToken();
      if (!token || !userDetails?._id) throw new Error("Unauthorized");

      // Fetch services created by employer
      return await EMPLOYER.fetchAllBookedWorkers({ pageParam, token });
    },
    retry: false,
    initialPageParam: 1,
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.header}>
          ...
          <CustomText
            baseFont={15}
            color={Colors?.white}
            style={{ opacity: 0.95, lineHeight: 22 }}
          >
            {t("employerBookingHeaderGuide")}
          </CustomText>
        ...
        <GradientWrapper height={Dimensions.get("window").height - 180}>
          {isLoading ? (
            ...
            <EmptyDataPlaceholder
              title="noCreatedServicesTitle"
              leftHeight={300}
              buttonTitle="createNewService"
              onPress={ClickCreateService}
              type="gradient"
            />
          )}
        </GradientWrapper>
